    ...
    <div class="col-lg">
        ...
        @foreach ($before as $item)
         <div class="alert alert-warning" role="alert">
            <h4 class="alert-heading">Informacja dotycząca Przeglądu technicznego!</h4>
            <p>Maszyna:{{$item['mark']}} {{$item['model']}} o rejesracji:<b>{{$item['machine_id']}}</b>. <br>
            {{$item['alertMessage']}} dnia:{{substr($item['date'],0,-9)}}
            </p>
            <hr>
            <p class="mb-0">Jeśli chcesz zmodyfikować dane dotyczące przegląu technicznego kliknij Przycisk <a class="btn btn-primary btn-sm text-decoration-none" href="/edit/{{$item['id']}}">Modyfikuj</a></p>
          </div> 
          @endforeach
           </div>
